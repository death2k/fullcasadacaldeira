...
class Pagination{
	...
	var $fullTagOpen	= '<div class="icp-pagination">';
	...
	function getAJAXlink( $count, $text) {
        ...
        $this->additionalParam = "{'action': 'pagination_action_icp','page' : $pageCount}";
		
	 /*   return "<a href=\"javascript:void(0);\" " . $this->anchorClass . "
				onclick=\"jQuery.post(ajaxurl, ". $this->additionalParam .", function(data){
					   jQuery('#". $this->contentDiv . "').html(data); }); \">"
			   . $text .'</a>';*/

			    return "<a href='#\' " . $this->anchorClass . "
				onclick=\"jQuery.ajax({
    type: 'POST',
    url: ajaxurl,
    traditional: true,
...
jQuery('.loading-overlay-icp').removeClass('hidden');
  jQuery('.loading-overlay-icp').addClass('visible');
         jQuery('.icp-text-load').html('Loading Page ". $text ."');
}
});return false;\">". $text ."</a>";
	}
}
